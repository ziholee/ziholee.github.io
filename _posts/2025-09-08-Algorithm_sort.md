...
정렬 알고리즘에는 여러 종류가 있으며, 각기 다른 성능 특성과 활용 분야를 가진다. 소스에서 언급된 주요 정렬 알고리즘들은 다음과 같다:
...
*   **병합 정렬 (Merge Sort)**
    ...
    *   **성능**:
        ...
        *   **특징**: 삽입 정렬보다 점근적으로 더 빠르며, **비교 정렬 중 점근적으로 최적(asymptotically optimal)** 이다. 하지만 MERGE 프로시저가 **제자리 정렬 방식이 아니다**. 소스에 따르면 안정 정렬(stable sort)입니다.
*   **힙 정렬 (Heapsort)**
...
*   **퀵 정렬 (Quicksort)**
    ...
    *   **성능**:
        ...
        *   **특징**: 최악의 경우 성능은 느리지만, 평균적으로 매우 효율적이어서 실용적인 정렬 방법으로 자주 선택된다. 힙 정렬보다 실제로는 더 좋은 성능을 보이는 경우가 많다. 또한 **제자리 정렬** 방식이다. 난수화된 버전인 **RANDOMIZED-QUICKSORT**는 특정 입력이 최악의 경우로 이어지지 않도록 한다. 소스에 따르면 안정 정렬이 아니다.
